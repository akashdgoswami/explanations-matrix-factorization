...
class ProbabilisticMatrixFactorization(object):
    ...
    def new_prediction_for(self, i, j, users=None, items=None):
        if users is None:
            users = self.users
        if items is None:
            items = self.items
    ...
    def do_fit(self, *args):
        kind = self.fit_type[0]
        args = self.fit_type[1:]
        if kind == 'batch':
            self.fit(*args)
    ...
    def fit_minibatches_until_validation(self, *args,  **kw):
        last_valid = np.inf
        stop_thresh=1e-3
...
def main():
    ...
    ratings = np.load('out2.npy')
    print ratings.shape
    print ratings.dtype
    print "Printing original ratings"
    print ratings
    ...
    pmf1.do_fit('mini-valid', 50, 50, 1e-3)

    #lls = []
    #for ll in pmf1.fit_lls():
    #    lls.append(ll)

    print "\n"

    random_ratings_matrix = helper.removeInterveneItemMatrix(ratings,9)
    print random_ratings_matrix.shape
    print random_ratings_matrix.dtype
    print "Printing single item randomized ratings"
    print random_ratings_matrix
